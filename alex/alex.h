...
class Alex {
 ...
 public:
  ...
  struct Stats {
    double space = 0.0;  // for calculate the space
    int num_keys = 0;
 ...
 public:
  ...
  explicit Alex(InputIterator first, InputIterator last,
    ...
    bulk_load(values.data(), static_cast<int>(values.size()));
    stats_.space = stats_.space * 1.0 / 1024.0 / 1024.0;
  }
...
// node's parent.
 ...
 public:
  ...
  void bulk_load(const V values[], int num_keys) {
    ...
    stats_.space += sizeof(root_node_) + sizeof(*root_node_);
    ...
    AlexNode<T, P>* cur = root_node_;
    auto node = static_cast<model_node_type*>(cur);
    std::vector<int> childSize(node->num_children_, 0);
    for (int i = 0; i < num_keys; i++) {
      double bucketID_prediction = node->model_.predict_double(values[i].first);
      int bucketID = static_cast<int>(bucketID_prediction);
      bucketID =
          std::min<int>(std::max<int>(bucketID, 0), node->num_children_ - 1);
      childSize[bucketID]++;
    ...
    //   std::cout << "i: " << i << ",\tsize: " << childSize[i] << std::endl;
    // }
  }
 ...
 private:
  ...
  void create_superroot() {
    ...
    stats_.space += sizeof(superroot_) + sizeof(*superroot_);
    stats_.space +=
        sizeof(superroot_->children_) + sizeof(*(superroot_->children_));
  ...
  void bulk_load_node(const V values[], int num_keys, AlexNode<T, P>*& node,
    ...
    if (num_keys <= derived_params_.max_data_node_slots *
      ...
      delete_node(node);
      node = data_node;
      stats_.space += sizeof(data_node) + sizeof(*data_node);
      return;
    }
    ...
    if (best_fanout_tree_cost < node->cost_ ||
      ...
      stats_.space += sizeof(model_node) + sizeof(*model_node);
      ...
      for (fanout_tree::FTNode& tree_node : used_fanout_tree_nodes) {
        ...
        stats_.space += sizeof(child_node) + sizeof(*child_node);

...
class AlexDataNode : public AlexNode<T, P> {
 public:
  ...
  int initialize(int num_keys, double density) {
    num_keys_ = num_keys;
    data_capacity_ =
        std::max(static_cast<int>(num_keys / density), num_keys + 1);
...
#endif
    int space = bitmap_size_ * 8 + data_capacity_ * (sizeof(T) + sizeof(P));
    return space;
  }

  // Assumes pretrained_model is trained on dense array of keys
  int bulk_load(const V values[], int num_keys,
                const LinearModel<T>* pretrained_model = nullptr,
                bool train_with_sample = false) {
    int space = initialize(num_keys, kInitDensity_);
    ...
    if (num_keys == 0) {
      ...
      return data_capacity_ * sizeof(T);
    }

...
MultiMap<KeyType, ValueType>::MultiMap(BidirIt first, BidirIt last,
  ...
  space += sizeof(data_) + data_.size() * sizeof(value_type);
  // Sort if necessary.
  if (!is_sorted) {
  ...
  rs_ = rsb.Finalize();
  space += sizeof(rs_) + rs_.radix_table_.size() * 4;
  space += rs_.spline_points_.size() * sizeof(Coord<KeyType>);
  space = space / 1024.0 / 1024.0;
  std::cout << "rs_.radix_table_.size(): " << rs_.radix_table_.size()
            << std::endl;
  std::cout << "rs_.spline_points_.size(): " << rs_.spline_points_.size()
            << std::endl;

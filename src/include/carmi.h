...
class CARMI {
 ...
 private:
  ...
  inline bool StorePrevious(
      const DataVectorType &dataset, int neededLeafNum,
  ...
  /**
   * @brief calculate the cost of the array node
   *
   * @param size the size of the data points
   * @param givenBlockNum the allocated number of data blocks
   * @return cost = time + kRate * space
   */
  double CalculateArrayCost(int size, int totalSize, int givenBlockNum);
   ...
   * @param arr leaf node
   */
  bool StoreData(int needLeafNum, int start_idx, int size,
 ...
 private:
  ...
  // std::map<IndexPair, BaseNode<KeyType>> remainingStruct;
  ...
  static const double kPLRRootSpace;  // the space cost of lr root

 public:
  friend class PLRType<DataVectorType, KeyType>;
...
#endif  // DEBUG
  if (readRate < 1) {
    readRate = std::max(readRate * 1.0, 0.95);
  }

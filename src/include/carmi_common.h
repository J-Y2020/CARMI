   ...
   public:
    ...
    inline iterator &operator++() {
      ...
      while (currunion < (currnode->array.flagNumber & 0x00FFFFFF) ||
        ...
        if (left + currunion >= tree->carmi_tree.entireData.size()) {
          *this = this->tree->end();
          return *this;
        } else if (currunion == (currnode->array.flagNumber & 0x00FFFFFF)) {
          currnode = &(tree->carmi_tree.entireChild[currnode->array.nextLeaf]);
          currslot = 0;
          currunion = 0;
          left = currnode->array.m_left;
          continue;
        } else if (currslot == tree->carmi_tree.kMaxSlotNum ||
                   tree->carmi_tree.entireData[left + currunion]
                           .slots[currslot]
                           .first == -DBL_MAX) {
          currslot = 0;
          currunion++;
        } else if (tree->carmi_tree.entireData[left + currunion]
                       .slots[currslot]
                       .first != -DBL_MAX) {
          return *this;
        }

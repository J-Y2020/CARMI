...
#include "../nodes/rootNode/trainModel/robust_lr.h"
#include "./dp.h"
...
void CARMI<KeyType, ValueType>::IsBetterRoot(int c, NodeType type,
  ...
  for (int i = 0; i < c; i++) {
    if (isPrimary) {
      space_cost += ceil(perSize[i].size * 1.0 / kMaxSlotNum) *
                    carmi_params::kMaxLeafNodeSizeExternal / 1024.0 / 1024.0;
    } else {
      space_cost += ceil(perSize[i].size * 1.0 / kMaxSlotNum) *
                    carmi_params::kMaxLeafNodeSize / 1024.0 / 1024.0;
    }
    if (perSize[i].size > readRate * kLeafMaxCapacity) {
...
TYPE CARMI<KeyType, ValueType>::ConstructRoot(const RootStruct &rootStruct,
  ...
  for (int i = 0; i < rootStruct.rootChildNum; i++) {
    if (subDataset->subInit[i].size < kMaxSlotNum * kMaxLeafNum)
      blockNum += ceil(subDataset->subInit[i].size * 1.0 / kMaxSlotNum);
  }

  root.fetch_model.SetBlockNumber(blockNum);

  entireData.resize(blockNum, LeafSlots<KeyType, ValueType>());
  entireDataSize = blockNum;
  nowDataSize = 0;
  return root;
}
